...
class OptOptions:
    def __init__(self, mu=1e4, mu_spec=1e4, rho=0.01,maxiter=100, maxiter_weight=20,
...
class IRLSolver:
    ...
    def solve_irl_pomdp_given_traj(self, traj, includeQuadCost=True):
        ...
        print("printing rewards for demonstrations,obtained policy, and reward name")
        ...
        for i in range(self._options.maxiter_weight):
            ...
            for r_name, val in weight.items():
                ...
                #print(rew_demo,rew_pol,r_name)
                #print(self._options.rho)
            ...
            for r_name, gradVal in diff_value_dict.items():
                ...
                new_weight[r_name] = weight[r_name] -(self._options.rho)* gradVal  # Gradient step size ?
    ...
    def compute_maxent_policy_via_scp(self, weight, init_problem=True, featMatch=None):
        ...
        for i in range(self._options.maxiter):
            ...
            self.scpOpt.setObjective(gp.LinExpr([*linExprReward, *penCostList]) - (self._options.rho/2)*quadExprReward, gp.GRB.MAXIMIZE)
