...
class IRLSolver:
	...
	def compute_maxent_policy_via_scp(self, weight, init_problem=True, featMatch=None):
		...
		if init_problem:
			...
			self.nu_s, self.nu_s_spec, self.nu_s_a, self.nu_s_a_spec, self.sigma, self.slack_spec,\
				self.slack_nu_p, self.slack_nu_n, self.slack_nu_p_spec, self.slack_nu_n_spec,\
					self.constrLin, self.constrLinSpec, self.constrTrustReg, self.constrTrustRegNu, self.constrTrustRegNuSpec,\
		...
		for i in range(self._options.maxiter):
			...
			self.update_constr_and_trust_region(self.scpOpt, self.constrLin, self.constrLinSpec, 
												self.constrTrustReg, self.constrTrustRegNu, self.constrTrustRegNuSpec,
												nu_s_k, nu_s_a_k, nu_s_spec_k, nu_s_a_spec_k, policy_k, 
	...
	def init_optimization_problem(self, mOpt, noLinearization=False, checkOpt=None):
		...
		constrTrustRegNu = self.constr_trust_region_nu_s_a(mOpt, nu_s_a)
		constrTrustRegNuSpec = self.constr_trust_region_nu_s_a(mOpt, nu_s_a_spec) if self._pomdp.has_sideinfo else dict()
		...
		return 	nu_s, nu_s_spec, nu_s_a, nu_s_a_spec, sigma, slack_spec, \
				...
				constrLin, constrLinSpec, constrTrustReg, constrTrustRegNu, constrTrustRegNuSpec,\
	...
	def constr_trust_region_nu_s_a(self, mOpt, nu_s_a):
		""" Parametrized the linear constraint by the trust region and returned
			the saved constraint to be modified later in the algorithm
			:param mOpt : Gurobi model
			:param sigma : observation based policy
		"""
		dictConstr = dict()
		for s, aDict in nu_s_a.items():
			for a, nu_s_a_val in aDict.items():
				dictConstr[(s,a,True)] = mOpt.addLConstr(nu_s_a_val, 
										gp.GRB.GREATER_EQUAL, 0, 
										name='trust_inf_nusa[{},{}]'.format(s,a))
				dictConstr[(s,a,False)] = mOpt.addLConstr(nu_s_a_val, 
										gp.GRB.LESS_EQUAL, 1.0, 
										name='trust_sup_nusa[{},{}]'.format(s,a))
		return dictConstr
	...
	def update_constr_and_trust_region(self, mOpt, constrLin, constrLinSpec, constrTrust,
											constrTrustRegNu, constrTrustRegNuSpec, 
											nu_s_past, nu_s_a_past, nu_s_past_spec, nu_s_a_spec_past,
											sigma_past, nu_s, nu_s_spec, sigma, currTrust):
		...
		for (s, a, tRhs), constrV in constrTrustRegNu.items():
			# tRhs is True correspons to lower bound of the trust region and inversely
			constrV.RHS = nu_s_a_past[s][a] * (1.0/currTrust if tRhs else currTrust)
		if self._pomdp.has_sideinfo:
			for (s, a, tRhs), constrV in constrTrustRegNuSpec.items():
				# tRhs is True correspons to lower bound of the trust region and inversely
				constrV.RHS = nu_s_a_spec_past[s][a] * (1.0/currTrust if tRhs else currTrust)
