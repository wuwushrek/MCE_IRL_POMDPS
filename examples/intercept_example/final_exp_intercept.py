...
pomdp_r_1 = parser_pomdp.PrismModel("intercept_5_2.pm", counter_type=stormpy.pomdp.PomdpMemoryPattern.selective_counter, memory_len=1, export=False)
...
options_opt = irl_solver.OptOptions(mu=1e3, mu_spec=1, mu_rew=1, maxiter=150, maxiter_weight=100,
                                    graph_epsilon=0, discount=0.999, verbose=True, verbose_solver=False)

# True reward in the POMDP environment
weight = {'finish' : 20, 'left' : 20, 'time' : 0.1}
...
pol_val_grb_1 = irlPb_1.from_reward_to_policy_via_scp(weight, save_info=(20, 'intercept_mem1_fwd', weight))
# Get the optimal policy if the agent has full observability
pol_val_mdp = irlPb_1.from_reward_to_optimal_policy_mdp_lp(weight, gamma=options_opt.discount, save_info=(-1,'intercept_mdp_fwd', weight))
...
_, pol_mdp_mem1_15 = irlPb_1.solve_irl_pomdp_given_traj(feat_mdp_15, save_info=(20, 'intercept_mem1_trajsize10mdp_irl', weight))
_, pol_pomdp_mem1_15 = irlPb_1.solve_irl_pomdp_given_traj(feat_pomdp_mem1_15, save_info=(20, 'intercept_mem1_trajsize10pomdp_irl', weight))

# Build the model with side information
pomdp_r_1_si = parser_pomdp.PrismModel("intercept_5_2.pm",  ["P=? [\"notbad\" U \"goal\"]"], counter_type=stormpy.pomdp.PomdpMemoryPattern.selective_counter, memory_len=1, export=False)
...
_, pol_mdp_mem1_5_si = irlPb_1_si.solve_irl_pomdp_given_traj(feat_mdp_15, save_info=(20, 'intercept_mem1_trajsize10mdp_irl_si', weight))
_, pol_pomdp_mem1_5_si = irlPb_1_si.solve_irl_pomdp_given_traj(feat_pomdp_mem1_15, save_info=(20, 'intercept_mem1_trajsize10pomdp_irl_si', weight))
