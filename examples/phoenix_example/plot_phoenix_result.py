...
def compare_perf_irl(output_file ='phoenix_demo_from_mdp_irl', demo_from_mdp=True, show=True):
	...
	if demo_from_mdp:
		file_names = ['phoenix_mdp_fwd', 'phoenix_mem1_fwd', 'phoenix_mem1_nosi', 'phoenix_mem1_si']
		labelMDP = r'$\mathrm{Opt. \ policy \  on \ MDP}$'
	else:
		file_names = ['avoid_mdp_fwd', 'avoid_mem1_fwd',
						'avoid_mem1_trajsize10pomdp_irl', 'avoid_mem1_trajsize10pomdp_irl_si']
		labelPOMDP = r'$\mathrm{Opt. \ FSC \  on \ POMDP, \ M=1, N=10}$'
	labels = [	labelMDP, 
				labelPOMDP,
				r'$\mathrm{Learned \ policy}$',
				r'$\mathrm{Guided \ policy}$',]
...
if __name__ == '__main__':
	compare_perf_irl(output_file ='phoenix_demo_from_mdp_irl', demo_from_mdp=True, show=True)
	# compare_perf_irl(output_file ='avoid_demo_from_pomdp_irl', demo_from_mdp=False, show=True)