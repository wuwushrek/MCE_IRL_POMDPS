	...
	for (indx, robot_traj) in traj_robot:
		...
		for (i,j,featv), (next_i, next_j, next_featv) in zip(robot_traj[:-1],robot_traj[1:]):
			if obsFullDict is None:
				...
				continue
			curr_obs = obsFullDict[(i-focus_zone[0], j-focus_zone[1], featv)]
...
for ind_traj_robot in robot_pos_evol:
	...
	arrowplot(plt.gca(), x[0:-1:traj_spacing], y[0:-1:traj_spacing], nArrs=nArrows, mutateSize=mutateSize, color=colorTrajectories, markerStyle='o')
